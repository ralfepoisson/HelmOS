...
- Idea Refinement now persists administrator-managed `idea_refinement_policies`, tracks per-proto-idea refinement lifecycle state directly on `proto_ideas`, and stores refined `idea_candidates` linked back to their proto-idea plus the applied conceptual tools and policy snapshot ids.
- Run one refinement pass locally with `npm run idea-refinement:run`. Set `IDEA_REFINEMENT_BATCH_SIZE=<n>` to process a small batch, `IDEA_REFINEMENT_RETRY_FAILED=true` to retry failed proto-ideas, and `IDEA_REFINEMENT_PROTO_IDEA_ID=<uuid>` to target one proto-idea directly.
